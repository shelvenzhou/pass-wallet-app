...
use crate::pass_logic::{Asset, Deposit, PassWalletManager, Subaccount, TokenType};
...
pub fn parse_command(command: &str) -> Result<Response, String> {
    let command_data: Command =
        serde_json::from_str(command).map_err(|e| format!("Failed to parse command: {}", e))?;
    ...
    match command_data {
        ...
        Command::CreatePassWallet { name, owner } => {
            match PASS_WALLET_MANAGER.create_wallet(name.clone(), owner.clone()) {
                Ok(wallet_address) => match PASS_WALLET_MANAGER.get_wallet_state(&wallet_address) {
        ...
        Command::InternalTransfer {
            ...
            match PASS_WALLET_MANAGER.internal_transfer(
                &wallet_address,
                &asset_id,
                amount,
                &from_subaccount,
                ...
                Ok(()) => {
                    ...
                    let from_balance = PASS_WALLET_MANAGER
                        .get_balance(&wallet_address, &from_subaccount, &asset_id)
                        .unwrap_or(0);
                    let to_balance = PASS_WALLET_MANAGER
                        .get_balance(&wallet_address, &to_subaccount, &asset_id)
                        .unwrap_or(0);
        ...
        Command::Withdraw {
            ...
            match PASS_WALLET_MANAGER.withdraw(
                &wallet_address,
                &asset_id,
                amount,
                &subaccount_id,
                ...
                Ok(()) => {
                    let remaining_balance = PASS_WALLET_MANAGER
        ...
        Command::GetProvenanceBySubaccount {
            wallet_address,
            subaccount_id,
        } => {
        ...
        Command::WithdrawToExternal {
            ...
            match PASS_WALLET_MANAGER.withdraw_to_external(
                ...
                Ok((signed_transaction, nonce, actual_gas_price, actual_gas_limit)) => {
                    Ok(Response {
                        success: true,
                        data: Some(serde_json::json!({
        ...
        Command::GetOutboxQueue => match PASS_WALLET_MANAGER.get_outbox_queue() {
