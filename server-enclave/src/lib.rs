...
use server_logic::{parse_command, Response};
...
async fn generate_handler(
    Json(_args): Json<Option<serde_json::Value>>,
) -> Result<JsonResponse<GenerateResponse>, (StatusCode, JsonResponse<ErrorResponse>)> {
...
) -> Result<JsonResponse<Vec<String>>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "List": null
    });
...
) -> Result<JsonResponse<SignResponse>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "Sign": {
...
// Create PASS wallet
async fn create_pass_wallet_handler(
    Json(request): Json<CreatePassWalletRequest>,
...
// List PASS wallets
async fn list_pass_wallets_handler(
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
...
// Get wallet state
async fn get_wallet_state_handler(
    Json(request): Json<GetWalletStateRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "GetPassWalletState": {
...
async fn get_assets_handler(
    Json(request): Json<GetAssetsRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "GetAssets": {
            "wallet_address": request.wallet_address
...
async fn get_provenance_log_handler(
    Json(request): Json<GetProvenanceLogRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "GetProvenanceLog": {
            "wallet_address": request.wallet_address
...
async fn get_provenance_by_asset_handler(
    Json(request): Json<GetProvenanceByAssetRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
...
async fn get_provenance_by_subaccount_handler(
    Json(request): Json<GetProvenanceBySubaccountRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
...
// Add subaccount
async fn add_subaccount_handler(
    Json(request): Json<AddSubaccountRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "AddSubaccount": {
            "wallet_address": request.wallet_address,
...
// Inbox deposit
async fn inbox_deposit_handler(
    Json(request): Json<InboxDepositRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "InboxDeposit": {
            "wallet_address": request.wallet_address,
...
// Claim inbox
async fn claim_inbox_handler(
    Json(request): Json<ClaimInboxRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "ClaimInbox": {
            "wallet_address": request.wallet_address,
...
// Internal transfer
async fn internal_transfer_handler(
...
async fn process_outbox_handler(
    Json(request): Json<ProcessOutboxRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "ProcessOutbox": {
            "wallet_address": request.wallet_address
...
async fn get_subaccount_balances_handler(
    Json(request): Json<GetSubaccountBalancesRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
...
// Sign GSM
async fn sign_gsm_handler(
    Json(request): Json<SignGSMRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "SignGSM": {
            "wallet_address": request.wallet_address,
...
async fn withdraw_to_external_handler(
    Json(request): Json<WithdrawToExternalRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "WithdrawToExternal": {
            "wallet_address": request.wallet_address,
...
async fn get_outbox_queue_handler(
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "GetOutboxQueue": {}
    });
...
async fn remove_from_outbox_handler(
    Json(request): Json<RemoveFromOutboxRequest>,
) -> Result<JsonResponse<serde_json::Value>, (StatusCode, JsonResponse<ErrorResponse>)> {
    let command = serde_json::json!({
        "RemoveFromOutbox": {
            "nonce": request.nonce
...
pub async fn run_server(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = Router::new()
        ...
        .route(
            "/pass/wallets/withdrawals/external",
            post(withdraw_to_external_handler),
        )
        .route("/pass/wallets/outbox", get(get_outbox_queue_handler))
        .route(
            "/pass/wallets/outbox/remove",
            post(remove_from_outbox_handler),
        )
        .route("/pass/wallets/balance", post(get_balance_handler))
        .route(
            "/pass/wallets/balances",
            post(get_subaccount_balances_handler),
        )
        .route("/pass/wallets/sign", post(sign_gsm_handler))
        // Provenance endpoints
        .route("/pass/wallets/provenance", post(get_provenance_log_handler))
        .route(
            "/pass/wallets/provenance/asset",
            post(get_provenance_by_asset_handler),
        )
        .route(
            "/pass/wallets/provenance/subaccount",
            post(get_provenance_by_subaccount_handler),
        )
        .layer(cors);

    println!("Pass Wallet server listening on port {}", port);
