...
impl PassWalletState {
    ...
    pub fn inbox_deposit(&mut self, deposit: Deposit) -> Result<()> {
        ...
        if self
            .inbox
            .iter()
            .any(|d| d.deposit_id == deposit.deposit_id)
        {
...
impl PassWalletManager {
    ...
    pub fn get_wallet_assets(&self, wallet_address: &str) -> Result<serde_json::Value> {
        ...
        for (asset_id, asset) in &wallet_state.assets {
            ...
            let total_balance: u64 = wallet_state
                .balances
                .iter()
    ...
    pub fn get_provenance_by_asset(
        &self,
        wallet_address: &str,
        asset_id: &str,
        ...
        let filtered_records: Vec<&ProvenanceRecord> = wallet_state
            .history
            .iter()
            .filter(|record| match &record.operation {
    ...
    pub fn get_provenance_by_subaccount(
        &self,
        wallet_address: &str,
        subaccount_id: &str,
    ) -> Result<serde_json::Value> {
    ...
    pub fn withdraw_to_external(
        ...
        let estimated_gas_limit = match asset.token_type {
            ...
            _ => {
                return Err(anyhow!(
                    "Withdrawal not supported for asset type: {:?}",
                    asset.token_type
                ));
            }
        };
...
mod tests {
    ...
    fn test_create_wallet_manager() {
        ...
        let wallet_address = manager
            .create_wallet("Test Wallet".to_string(), "alice".to_string())
            .unwrap();
    ...
    fn test_multiple_wallets() {
        ...
        let wallet1 = manager
            .create_wallet("Wallet 1".to_string(), "alice".to_string())
            .unwrap();
        let wallet2 = manager
            .create_wallet("Wallet 2".to_string(), "bob".to_string())
            .unwrap();
    ...
    fn test_wallet_operations() {
        ...
        manager
            .add_asset(&wallet_address, "eth".to_string(), asset)
            .unwrap();
        ...
        // Test claim
        manager
            .claim_inbox(&wallet_address, "deposit1", "sub1")
            .unwrap();
