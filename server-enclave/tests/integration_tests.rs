...
impl IntegrationTestEnvironment {
    ...
    fn create_wallet(&mut self, name: &str, owner: &str) -> Result<String> {
        let address = self
            .manager
            .create_wallet(name.to_string(), owner.to_string())?;
    ...
    fn add_asset(&mut self, wallet_name: &str, symbol: &str, asset: Asset) -> Result<()> {
        ...
        self.manager
            .add_asset(wallet_address, asset_id.clone(), asset)?;
        self.assets
            .insert(format!("{}_{}", symbol, wallet_name), asset_id);
        Ok(())
    }
    ...
    fn process_deposit(
        ...
        let asset_id = self
            .assets
            .get(&format!("{}_{}", asset_symbol, wallet_name))
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "Asset not found: {} for wallet {}",
                    asset_symbol,
                    wallet_name
                )
            })?;
        ...
        let deposit = Deposit {
            ...
            transaction_hash: format!(
                "0x{}",
                hex::encode(format!("{}_{}", wallet_name, deposit_id))
            ),
        ...
        self.manager
            .claim_inbox(wallet_address, deposit_id, subaccount_id)?;
    ...
    fn get_balance(
        ...
        self.manager
            .get_balance(wallet_address, subaccount_id, asset_id)
    }
    ...
    fn withdraw(
        ...
        self.manager
            .withdraw(wallet_address, asset_id, amount, subaccount_id, destination)
    }
...
mod integration_tests {
    ...
    fn test_multi_user_trading_scenario() -> Result<()> {
        ...
        assert_eq!(
            env.get_balance("alice_wallet", "main", "ETH")?,
            10000000000000000000
        );
        assert_eq!(env.get_balance("bob_wallet", "main", "USDC")?, 50000000000);
        assert_eq!(
            env.get_balance("charlie_wallet", "main", "ETH")?,
            5000000000000000000
        );
        assert_eq!(
            env.get_balance("charlie_wallet", "main", "USDC")?,
            25000000000
        );
        ...
        assert_eq!(
            env.get_balance("alice_wallet", "trading", "ETH")?,
            3000000000000000000
        ); // 3 ETH
        assert_eq!(
            env.get_balance("alice_wallet", "savings", "ETH")?,
            2000000000000000000
        ); // 2 ETH

        assert_eq!(env.get_balance("bob_wallet", "main", "USDC")?, 15000000000); // 15,000 USDC remaining
        assert_eq!(
            env.get_balance("bob_wallet", "trading", "USDC")?,
            20000000000
        ); // 20,000 USDC
        assert_eq!(
            env.get_balance("bob_wallet", "savings", "USDC")?,
            15000000000
        ); // 15,000 USDC
        ...
        assert_eq!(
            env.get_balance("bob_wallet", "trading", "USDC")?,
            15000000000
        ); // 15,000 USDC remaining
        assert_eq!(
            env.get_balance("charlie_wallet", "trading", "ETH")?,
            1500000000000000000
        ); // 1.5 ETH remaining
        assert_eq!(
            env.get_balance("charlie_wallet", "trading", "USDC")?,
            8000000000
        ); // 8,000 USDC remaining
        ...
        for wallet_name in ["alice_wallet", "bob_wallet", "charlie_wallet"] {
            ...
            assert!(
                !wallet_state.outbox.is_empty(),
                "Wallet {} should have outbox entries",
                wallet_name
            );
        }
    ...
    fn test_complex_multi_asset_workflow() -> Result<()> {
            ...
            (
                ...
                Asset {
                    ...
                    contract_address: Some(
                        "0xa0b86a33e6776e7bb8c4c9f8d9b2d5f1c4e3f1d2".to_string(),
                    ),
                    token_id: None,
                    symbol: "USDC".to_string(),
                    name: "USD Coin".to_string(),
        ...
        // Verify initial deposits
        assert_eq!(
            env.get_balance("enterprise_wallet", "treasury", "ETH")?,
            10000000000000000000
        );
        assert_eq!(
            env.get_balance("enterprise_wallet", "treasury", "USDC")?,
            1000000000000
        );
        assert_eq!(
            env.get_balance("enterprise_wallet", "treasury", "WBTC")?,
            500000000
        );
        assert_eq!(
            env.get_balance("enterprise_wallet", "operations", "ETH")?,
            2000000000000000000
        );
        assert_eq!(
            env.get_balance("enterprise_wallet", "operations", "USDC")?,
            200000000000
        );
        ...
        let withdrawals = vec![
            (
                ...
                "0xdev1111111111111111111111111111111111111",
            ), // 1 ETH
            (
                "development",
                "USDC",
                100000000000u64,
                "0xdev2222222222222222222222222222222222222",
            ), // 100,000 USDC
            (
                "marketing",
                "ETH",
                500000000000000000u64,
                "0xmkt1111111111111111111111111111111111111",
            ), // 0.5 ETH
            (
                "marketing",
                "USDC",
                75000000000u64,
                "0xmkt2222222222222222222222222222222222222",
            ), // 75,000 USDC
            (
                "operations",
                "ETH",
                500000000000000000u64,
                "0xops1111111111111111111111111111111111111",
            ), // 0.5 ETH
            (
                "operations",
                "USDC",
                25000000000u64,
                "0xops2222222222222222222222222222222222222",
            ), // 25,000 USDC
        ];
        ...
        assert_eq!(
            env.get_balance("enterprise_wallet", "development", "ETH")?,
            2500000000000000000
        ); // 3+0.5-1 = 2.5 ETH
        assert_eq!(
            env.get_balance("enterprise_wallet", "development", "USDC")?,
            250000000000
        ); // 300+50-100 = 250k USDC

        assert_eq!(
            env.get_balance("enterprise_wallet", "marketing", "ETH")?,
            1500000000000000000
        ); // 2-0.5 = 1.5 ETH
        assert_eq!(
            env.get_balance("enterprise_wallet", "marketing", "USDC")?,
            125000000000
        ); // 200-75 = 125k USDC
        ...
        assert_eq!(
            env.get_balance("enterprise_wallet", "reserves", "ETH")?,
            2500000000000000000
        ); // 2.5 ETH
        assert_eq!(
            env.get_balance("enterprise_wallet", "reserves", "USDC")?,
            250000000000
        ); // 250k USDC
        assert_eq!(
            env.get_balance("enterprise_wallet", "reserves", "WBTC")?,
            100000000
        ); // 1 BTC
        ...
        assert!(
            wallet_state.history.len() > 15,
            "Should have extensive transaction history"
        );

        // Verify provenance filtering works with complex history
        let eth_provenance = env.manager.get_provenance_by_asset(
            wallet_address,
            &env.assets.get("ETH_enterprise_wallet").unwrap(),
        )?;
        let usdc_provenance = env.manager.get_provenance_by_asset(
            wallet_address,
            &env.assets.get("USDC_enterprise_wallet").unwrap(),
        )?;
        let wbtc_provenance = env.manager.get_provenance_by_asset(
            wallet_address,
            &env.assets.get("WBTC_enterprise_wallet").unwrap(),
        )?;

        // Verify each asset has appropriate number of transactions
        let eth_records: Vec<serde_json::Value> =
            serde_json::from_value(eth_provenance["provenance_records"].clone())?;
        let usdc_records: Vec<serde_json::Value> =
            serde_json::from_value(usdc_provenance["provenance_records"].clone())?;
        let wbtc_records: Vec<serde_json::Value> =
            serde_json::from_value(wbtc_provenance["provenance_records"].clone())?;

        assert!(eth_records.len() >= 8, "ETH should have many transactions");
        assert!(
            usdc_records.len() >= 8,
            "USDC should have many transactions"
        );
        assert!(
            wbtc_records.len() >= 2,
            "WBTC should have fewer transactions"
        );
    ...
    fn test_high_frequency_micro_transactions() -> Result<()> {
        ...
        assert_eq!(
            total_balance, expected_remaining,
    ...
    fn test_cross_wallet_simulation() -> Result<()> {
        ...
        for (wallet_name, _) in &wallets {
            ...
            let wallet_state = env.manager.get_wallet(wallet_address).unwrap();
            assert!(
                !wallet_state.history.is_empty(),
                "Wallet {} should have transaction history",
                wallet_name
            );
        }
    ...
    fn test_error_recovery_scenarios() -> Result<()> {
        ...
        let transfer_result =
            env.transfer("test_wallet", "ETH", 15000000000000000000, "main", "backup");
        assert!(
        ...
        assert_eq!(
            env.get_balance("test_wallet", "main", "ETH")?,
            10000000000000000000
        );
        ...
        let withdraw_result =
            env.withdraw("test_wallet", "ETH", 15000000000000000000, "main", "0x1234");
        assert!(
        ...
        env.manager
            .inbox_deposit(wallet_address, deposit1.clone())?;
        ...
        let claim_result = env
            .manager
            .claim_inbox(wallet_address, "nonexistent", "main");
        assert!(
            claim_result.is_err(),
            "Claiming non-existent deposit should fail"
        );
    ...
    fn test_withdrawal_gas_calculation_fix() -> Result<()> {
        ...
        assert_eq!(
            env.get_balance("gas_test_wallet", "main", "ETH")?,
            initial_eth_amount
        );
        assert_eq!(
            env.get_balance("gas_test_wallet", "main", "USDC")?,
            initial_usdc_amount
        );
        ...
        assert!(
            result.is_ok(),
            "ETH withdrawal should succeed with sufficient balance for gas"
        );
        ...
        let eth_balance_before_usdc_withdrawal =
            env.get_balance("gas_test_wallet", "main", "ETH")?;
        ...
        assert!(
            result.is_ok(),
            "USDC withdrawal should succeed with sufficient ETH for gas"
        );
        let (_, _, gas_price, gas_limit) = result.unwrap();
        let gas_cost = gas_price * gas_limit;
        ...
        assert_eq!(
            env.get_balance("gas_test_wallet", "main", "ETH")?,
            expected_eth_balance_after_usdc
        );
        ...
        assert!(
            result.is_err(),
            "Should fail when trying to withdraw all ETH without leaving gas"
        );
        assert!(result
            .err()
            .unwrap()
            .to_string()
            .contains("Insufficient ETH for withdrawal + gas fees"));
        ...
        if result.is_err() {
            assert!(result
                ...
                .unwrap()
                .to_string()
                .contains("Insufficient ETH for gas fees"));
        }
