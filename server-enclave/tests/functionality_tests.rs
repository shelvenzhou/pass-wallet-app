...
use server_enclave::pass_logic::{
    Asset, Deposit, PassWalletManager, Subaccount, TokenType, TransactionOperation,
};
...
impl TestEnvironment {
    ...
    fn new() -> Result<Self> {
        ...
        // Create wallet
        let wallet_address =
            manager.create_wallet("Test Wallet".to_string(), "test_user".to_string())?;
...
mod tests {
    ...
    fn test_claim_deposit_flow() -> Result<()> {
        ...
        // Claim the deposit
        env.manager
            .claim_inbox(&env.wallet_address, "deposit_1", &env.main_subaccount)?;
    ...
    fn test_internal_transfer_flow() -> Result<()> {
        ...
        env.manager.inbox_deposit(&env.wallet_address, deposit)?;
        env.manager
            .claim_inbox(&env.wallet_address, "deposit_2", &env.main_subaccount)?;
    ...
    fn test_withdraw_flow() -> Result<()> {
        ...
        env.manager.inbox_deposit(&env.wallet_address, deposit)?;
        env.manager
            .claim_inbox(&env.wallet_address, "deposit_3", &env.main_subaccount)?;
    ...
    fn test_end_to_end_workflow() -> Result<()> {
        ...
        env.manager
            .inbox_deposit(&env.wallet_address, eth_deposit)?;
        env.manager
            .inbox_deposit(&env.wallet_address, usdc_deposit)?;

        // Step 2: Claim deposits to main account
        env.manager
            .claim_inbox(&env.wallet_address, "eth_deposit", &env.main_subaccount)?;
        env.manager
            .claim_inbox(&env.wallet_address, "usdc_deposit", &env.main_subaccount)?;
    ...
    fn test_insufficient_balance_scenarios() -> Result<()> {
        ...
        env.manager.inbox_deposit(&env.wallet_address, deposit)?;
        env.manager
            .claim_inbox(&env.wallet_address, "deposit_4", &env.main_subaccount)?;
    ...
    fn test_duplicate_deposit_rejection() -> Result<()> {
        ...
        let deposit =
            env.create_deposit(&env.eth_asset_id, 1000000000000000000, "duplicate_deposit");

        // First deposit should succeed
        env.manager
            .inbox_deposit(&env.wallet_address, deposit.clone())?;
    ...
    fn test_multi_asset_balances() -> Result<()> {
        ...
        env.manager
            .claim_inbox(&env.wallet_address, "multi_eth", &env.main_subaccount)?;
        env.manager
            .claim_inbox(&env.wallet_address, "multi_usdc", &env.main_subaccount)?;

        // Get all balances for main subaccount
        let balances = env
            .manager
            .get_subaccount_balances(&env.wallet_address, &env.main_subaccount)?;
    ...
    fn test_provenance_filtering() -> Result<()> {
        ...
        env.manager
            .claim_inbox(&env.wallet_address, "prov_eth", &env.main_subaccount)?;
        env.manager
            .claim_inbox(&env.wallet_address, "prov_usdc", &env.trading_subaccount)?;
        ...
        let eth_provenance = env
            .manager
            .get_provenance_by_asset(&env.wallet_address, &env.eth_asset_id)?;
        ...
        let usdc_provenance = env
            .manager
            .get_provenance_by_asset(&env.wallet_address, &env.usdc_asset_id)?;
        ...
        let main_provenance = env
            .manager
            .get_provenance_by_subaccount(&env.wallet_address, &env.main_subaccount)?;
        let main_records: Vec<serde_json::Value> =
            serde_json::from_value(main_provenance["provenance_records"].clone())?;
        assert_eq!(main_records.len(), 2); // ETH claim + ETH transfer (from main)

        let trading_provenance = env
            .manager
            .get_provenance_by_subaccount(&env.wallet_address, &env.trading_subaccount)?;
        let trading_records: Vec<serde_json::Value> =
            serde_json::from_value(trading_provenance["provenance_records"].clone())?;
